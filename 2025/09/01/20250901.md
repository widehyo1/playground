...
' _ {} "$parsed_html_dir" > "${result_dir}/xargs_lynx_out.csv"
end=$(date +%s%3N)
elapsed=$((end - start))
...
find "${tmpfs_dir}" -name "*.csv" | sort -n | xargs -I @ cat "${tmpfs_dir}/@" > "${result_dir}/xargs_lynx_out.csv"
echo "compressing csv files..."
tar --zstd -cf "${result_dir}/xargs_lynx_out.zst" "${result_dir}"/*.csv
end=$(date +%s%3N)
elapsed=$((end - start))
echo "[xargs, lynx] Time taken: ${elapsed} milliseconds"
