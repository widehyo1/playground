...
~/gitclone/playground/content_base/cli/pipeline/geeknews $ csvlook split_workspace/sp_128/output_02.csv
ValueError: Row 21 has 5 values, but Table only has 3 columns.



```awk
{
  basename = substr($0,1,length($0)-4)
  clean_name = basename "_clean.csv"
  err_name = basename "_err.csv"
  printf "csvclean -a %s 1>%s 2>%s\n", $0, clean_name, err_name
}
```
...
```awk
{
  basename = substr($0,1,length($0)-4)
  clean_name = basename "_clean.csv"
  printf "csvclean -a %s 1>/dev/null\n", clean_name
}
```
...

{
  basename = substr($0,1,length($0)-4)
  clean_name = basename "_clean.csv"
  printf "\\COPY geeknews_xargs (url, title, content) FROM '%s' WITH (FORMAT csv);\n", clean_name
}


~/gitclone/playground/content_base/cli/pipeline/geeknews $ !!
time bash parse_htmlq_sequential.sh
...
18666  2025-08-31 23:58:42 man csvclean
18667  2025-09-01 00:02:01 csvclean bulk_data_seq.csv --omit-error-rows
18668  2025-09-01 00:02:19 csvclean -a --omit-error-rows bulk_data_seq.csv | wc
18669  2025-09-01 00:02:28 csvclean -a bulk_data_seq.csv | wc
18670  2025-09-01 00:02:39 csvclean -a --omit-error-rows bulk_data_seq.csv 2>/dev/null | wc
18671  2025-09-01 00:02:48 csvclean -a bulk_data_seq.csv 2>/dev/null | wc
18672  2025-09-01 00:03:11 csvclean -a --omit-error-rows bulk_data_seq.csv 2>/dev/null 1>clean.csv
18673  2025-09-01 00:03:24 wc clean.csv
18674  2025-09-01 00:03:45 csvclean -a --omit-error-rows clean.csv 2>/dev/null | wc
18675  2025-09-01 00:03:56 csvlook clean.csv
18676  2025-09-01 00:04:06 ll
18677  2025-09-01 00:04:21 cp bulk_insert.sql insert_clean_csv.sql
18678  2025-09-01 00:04:24 vi insert_clean_csv.sql
18679  2025-09-01 00:04:47 cp bulk_insert.sql insert_clean_csv.sh
18680  2025-09-01 00:04:50 vi insert_clean_csv.sh
18681  2025-09-01 00:05:02 cp bulk_insert.sh insert_clean_csv.sh
18682  2025-09-01 00:05:05 vi insert_clean_csv.sh
18683  2025-09-01 00:05:17 bash insert_clean_csv.sh
18684  2025-09-01 00:05:27 bash delete_xargs.sh
18685  2025-09-01 00:05:36 bash insert_clean_csv.sh
18686  2025-09-01 00:08:13 history 100
